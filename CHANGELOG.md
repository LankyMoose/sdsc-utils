...
- Reopen the start screen anytime with a configurable controller chord (default **PS**); record or reset the gesture in Settings → **Start screen** (when start screen is enabled).
- Settings → **Start screen**: enable toggle, reopen-gesture recording, and UI sounds (nav / action / hold-complete) with volume. Catalog persists in `games.json`.
...
- DualSense HID I/O for the tray daemon runs on a dedicated **hid-worker** thread (lightbar / battery poll / Identify / power-off **and** start-nav input). The UI PadPoll path only clones a shared input snapshot — it never opens HID — so Identify flashes can interleave with short input reads on the same handle. PadPoll and idle HID sampling run at ~16 ms (~60 Hz) while the start screen is open or a reopen gesture is being recorded; otherwise they stay at ~50 ms.
- Start-screen discrete actions (face buttons, Options, L2/R2, Enter/Escape) fire on press-down; holds are cancelled by other input while in progress.
- Games ↔ Controllers slide runs ~220 ms; mid-anim pad poll skips controller-row rebuilds but still accepts L2/R2 (and Circle) so slides stay interruptible.
- With the start screen enabled and no controllers known, presence / unread poll runs every ~500 ms so Bluetooth 0→1 open is faster (restores the 3 s cadence once a pad is connected).
...
- List keep-in-view scrolls only on the leading edge (down pins bottom, up pins top); wrap-around flips the pin. Games footer shows Edit/Save, Close/Cancel, and sort (muted in edit); **Add shortcut…** only in edit mode. Selected game persists across edit enter/exit by identity; enter/Cancel prefer centering the selection, and Save centers when it is out of view. Start-screen window is 640×500 with tighter header/footer chrome. Start-screen settings live under Settings → **Start screen** (not System).

### Fixed

- Start-screen pad nav stays on DualSense HID via hid-worker snapshot reads (same `hid:{serial}` id through Identify); the UI never blocks on HID open/read, so Identify flashes and hold/slide input can stay live together.
- Start-screen pad navigation (and UI sounds) arms only after the window is shown (`StartOpened`), so nav clicks cannot fire before the UI is visible.
...
- Identify / lightbar no longer stall the UI during start-screen pad polling (hid-worker owns HID; PadPoll is an O(1) snapshot clone).
- Start-screen slide animation skips controller-row rebuilds mid-transition; L2/R2 still reverse or complete the slide.
...
- Start-screen hold actions (close game / power off / replace Proceed) commit when the hold reaches full duration while still held; any other input cancels an in-progress hold.
