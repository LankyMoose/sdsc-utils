...
- Browse mode **Options** toggles sort between **Last played** and **A–Z** (preference in prefs). Successful launches record `last_played_ms`. Manual shortcuts support optional launch arguments and a custom icon path.
- Games ↔ Controllers header (L2/R2 to switch); item actions on the selected row; footer for Edit / Close / Options. Steam icons use the modern `librarycache/{appid}/` layout (with legacy flat-name fallback); game art uses a shared 2:3 portrait cell.
- Discrete actions fire on press-down; hold actions (close game / power off / replace Proceed) complete when the hold reaches full duration while still held. Nav and sounds arm only after the window is shown.
