...
fn dedupe_statuses(mut statuses: Vec<ControllerStatus>) -> Vec<ControllerStatus> {
    ...
    for status in statuses.drain(..) {
        ...
        {
            if status.connection.is_usb() && !existing.connection.is_usb() {
                *existing = status;
            }
        } else {
            unique.push(status);
...
pub fn power_off_bluetooth_timed(
    ...
    Result<(), String>,
    crate::controller::dualsense::lightbar::HidPhaseTiming,
) {
    let mut timing = crate::controller::dualsense::lightbar::HidPhaseTiming::default();
...
fn power_off_bluetooth_unlocked_timed(
    api: &HidApi,
    serial: &str,
    timing: &mut crate::controller::dualsense::lightbar::HidPhaseTiming,
) -> Result<(), String> {
    let enum_started = std::time::Instant::now();
...
mod tests {
    ...
    fn low_battery_requires_discharging() {
        let discharging = dualsense_status(
            1,
            "DualSense",
    ...
    fn dedupe_prefers_usb_for_same_identity() {
        ...
        assert_eq!(charging.connection, Connection::Usb);
    }
