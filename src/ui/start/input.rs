...
use crate::ui::start::gesture::{GestureControl, GestureDetector};
...
fn read_one_report(
    ...
    let bus = if is_bluetooth {
        crate::controller::model::Connection::Bluetooth
    } else {
        crate::controller::model::Connection::Usb
    };
    Ok(Some(
        crate::controller::driver::registry()
            .driver_for_kind(crate::controller::model::ControllerKind::DualSense)
            .map(|drv| drv.parse_input(&buf, bus))
            .unwrap_or_else(|| {
                crate::controller::dualsense::input::parse_report(
                    &buf,
